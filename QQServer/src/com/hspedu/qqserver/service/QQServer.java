    ...
    public QQServer() {
        ...
        try {
            ...
            while (true) { //当和某个客户端连接后，会继续监听, 因此while
                Socket socket = ss.accept();//如果没有客户端连接，就会阻塞在这里。连接了说明发过来了checkuser
                //得到socket关联的对象输入流
                ...
                if (checkUser(u.getUserId(), u.getPasswd())) {//登录通过
                   ...
                   //将message对象回复客户端
                    ...
                    // 检测未读消息,并发送
                    if((messages = StoreUnreadMessages.massageBuf.get(u.getUserId()))!=null){
                    ...
                    // 清空未读消息
                    StoreUnreadMessages.massageBuf.remove(u.getUserId());
